...
class Tensor {
  private:
    ...
    // strides tell how many elements to skip in the flat buffers, to advance one step along each
    // axis
  ...
  public:
    ...
    // mmap ctor
    Tensor(std::vector<int> shape, const float* data, std::shared_ptr<void> owner);
