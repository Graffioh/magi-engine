...
MappedFile::MappedFile(const std::string& file_path) {
    ...
    // mmap!
    // we "map" a st_size bytes of the file into our virtual address space (tracked by kernel via
    // page tables), that will be used to quickly refer to model weights  (loaded on first forward
    // pass, with page faults) without extra overhead (no I/O, lazy loading)
