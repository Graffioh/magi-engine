...
static std::string write_dummy_file(const std::vector<float>& vals) {
    ...
    out.write(reinterpret_cast<const char*>(vals.data()), static_cast<std::streamsize>(vals.size() * sizeof(float)));
    out.close();
    return p.string();
}
...
static void check_floats(TestState&                s,
...
void run_mmap_tests(TestState& s) {
    ...
    check(s, "missing file throws", std::vector<int>{ threw ? 1 : 0 }, std::vector<int>{ 1 });

    MappedFile mf(path);

    // Size is in bytes: 6 floats * 4 bytes = 24.
    check(s, "mmap size in bytes", std::vector<int>{ static_cast<int>(mf.size()) }, std::vector<int>{ 24 });
