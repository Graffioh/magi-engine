...
inline void check(TestState&                s,
    ...
    if ((int) expected.size() != total) {
        std::cout << "  [FAIL] " << name << ": expected size " << expected.size() << " but tensor has " << total
                  << " elements\n";
        s.failed++;
        return;
    }
    ...
    if (ok) {
        ...
        std::cout << "  expected=[";
        for (int i = 0; i < total; ++i) {
            std::cout << expected[i] << (i + 1 < total ? ", " : "");
        }
        std::cout << "]  got=[";
        for (int i = 0; i < total; ++i) {
            std::cout << got.data_ptr()[i] << (i + 1 < total ? ", " : "");
        }
        std::cout << "]\n";
        s.passed++;
    } else {
